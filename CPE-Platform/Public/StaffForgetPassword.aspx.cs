...
{
	public partial class StaffForgetPassword : System.Web.UI.Page
	{
		...
		protected void btnConfirmStaffForgetPassword_Click(object sender, EventArgs e)
		{
			if (Page.IsValid)
			{
				...
				{
					SqlCommand cmd = new SqlCommand("spResetPasswordStaff", con);
					cmd.CommandType = CommandType.StoredProcedure;

					//SqlParameter paramStudentID = new SqlParameter("@StudentId", txtStudentID.Text);
					SqlParameter paramStaffIC = new SqlParameter("@StaffIc", txtStaffIC.Text);

					//cmd.Parameters.Add(paramStudentID);
					cmd.Parameters.Add(paramStaffIC);
					...
					while (rdr.Read())
					{
						...
						{
							SendPasswordResetEmail(rdr["Email"].ToString(), rdr["StaName"].ToString(), rdr["UniqueId"].ToString());
		...
		private void SendPasswordResetEmail(string ToEmail, string studentName, string UniqueId)
		{
			...
			sbEmailBody.Append("<br/>"); sbEmailBody.Append("http://localhost:62893/Public/StaffResetPassword?uid=" + UniqueId);
